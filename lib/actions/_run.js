...
/** FormData → plain object. Uses the LAST value so a hidden "false" input
 *  followed by a checked checkbox "true" resolves to "true". */
export const fields = (fd, keys) =>
  Object.fromEntries(keys.filter((k) => fd.has(k)).map((k) => [k, fd.getAll(k).at(-1)]));
