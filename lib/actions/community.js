...
const projectForm = (formData) => {
  ...
  for (const k of PROJECT_FIELDS) if (formData.has(k)) fd.append(k, formData.getAll(k).at(-1));
...
export async function createUser(formData) {
  ...
  if (!body.passwordConfirm) body.passwordConfirm = body.password; // admin sets one temporary password
